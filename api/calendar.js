...
function getTTL(week, events) {
  ...
  // If any High-impact event from yesterday is STILL pending → keep refreshing
  const yesterday = new Date(); yesterday.setDate(yesterday.getDate()-1);
  const yd = yesterday.toISOString().slice(0,10);
  const pendingYesterday = events.some(e =>
    e.date?.slice(0,10) === yd &&
    e.actual === null &&
    e.impact === 'High' &&
    new Date(e.date).getTime() < nowMs
  );
  if (pendingYesterday) {
    console.log('[calendar] ⚡ High-impact event from yesterday still pending — using TTL_RECENT');
    return TTL_RECENT;
  }
...
function nn(n) {
  ...
  let s = n.toLowerCase()
    ...
    .replace(/\bcips\b/gi, '')
    .trim();
  // Expand acronyms BEFORE suffix stripping — makes FF 'PPI' match RA 'Producer Price Index'
  s = s.replace(/\bcpi\b/g, 'consumer price index');
  ...
  // 'PPI Final Demand' variants
  s = s.replace(/producer price index final demand/gi, 'producer price index final demand');
...
function bestMatch(ffEv, idx, getName, getCC, minScore = 0.42) {
  ...
  // ── Keyword fallback for macro key events (CPI/PPI/NFP/PMI/GDP) ──────
  // When country + date match but fuzzy sim < threshold, check if both share
  // the same macro keyword (after acronym expansion) — if so, force match.
  if (best && ffEv.country === cc) {
    ...
    const nFF = nn(ffEv.event);
    const nRA = nn(getName(best));
    const sharedKey = MACRO_KEYS.find(k => nFF.includes(k) && nRA.includes(k));
    if (sharedKey) {
      console.log(`[match] ⚡ keyword-fallback "${ffEv.event}" ↔ "${getName(best)}" via "${sharedKey}"`);
      return { ev: best, score: 0.50, forced: true };
    }
  }
...
function mergeActuals(ffEvents, raEvents, fxsEvents) {
  ...
  const result = ffEvents.map(ffEv => {
    ...
    if (raNorm.length > 0) {
      ...
      if (ffEv.impact === 'High') {
        console.log(`[MISS][MACRO] ${ffEv.event} | ${ffEv.country} | ${ffEv.date} | impact=High`);
      }
      }
...
function normalizeEvent(ev, nowMs, regime = {}) {
  ...
  // Dynamic scoring — same engine as Market Pulse
  const eventTitle = ev.event || ev.title || ev.name || '';
  const dynScored  = scoreNews(eventTitle, { actual, forecast: estimate, previous }, regime);

  return {
    date:         ev.date        || '',
    event:        eventTitle,
    country:      ev.country     || '',
    impact:       ev.impact      || 'Low',    // preserved — used by UI filters
    actual,
    estimate,
    previous,
    unit:         ev.unit        || '',
    ...
    // Dynamic fields (new — from newsScoringEngine)
    dynamicScore:    dynScored?.score      ?? null,
    dynamicLabel:    dynScored?.label      ?? null,
    dynamicColor:    dynScored?.color      ?? null,
    dynamicCategory: dynScored?.category   ?? null,
    dynamicAssets:   dynScored?.assets     ?? [],
    dynamicDirection:dynScored?.direction  ?? 'neutral',
  };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const eventTitles = events.map(e => e.event || e.title || e.name || '').filter(Boolean);
    const calRegime   = inferMarketRegime(eventTitles);
    console.log('[calendar] regime:', JSON.stringify(calRegime));

    const payload = [...events].map(ev => normalizeEvent(ev, nowMs, calRegime));
