  ...
  return (
    <div style={{
      ...
      <div style={{ marginBottom: 16 }}>
        ...
        {/* Threshold markers — number + label stacked, no overlap */}
        <div style={{ position: 'relative', height: 22, marginTop: 2 }}>
          {[{ pct: 45, num: '45', lbl: 'MED' }, { pct: 70, num: '70', lbl: 'HIGH' }].map(({ pct, num, lbl }) => (
            <div key={pct} style={{
              ...
              <div style={{ width: 1, height: 4, background: T.border, flexShrink: 0 }} />
              <span style={{ fontSize: 8, color: sub2, fontWeight: 600, whiteSpace: 'nowrap', lineHeight: 1 }}>{num}</span>
              <span style={{ fontSize: 7, color: sub2, opacity: 0.6, whiteSpace: 'nowrap', lineHeight: 1, letterSpacing: '0.03em' }}>{lbl}</span>
            </div>
          ))}
        </div>
      </div>
