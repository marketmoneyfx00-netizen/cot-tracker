...
// Premium password confirmation toast (used after updatePassword success)
const PasswordToast = ({ visible }) => {
  ...
  return (
    <div style={{
      ...
      <div>
        <p style={{ margin: '0 0 3px', fontSize: 14, fontWeight: 700, color: '#14532d', letterSpacing: '-0.1px' }}>
...
export default function LoginScreen() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      setShowToast(false);
      // If in reset flow, redirect to password login after toast
      if (mode === 'reset') {
        setSuccess('');
        setError('');
        setMode('password');
