...
export default function OnboardingModal({ profile, onComplete }) {
  ...
  const [isMobile, setIsMobile] = useState(() => {
    // Safe SSR guard (not needed for Vite but defensive)
  ...
  // try/finally ensures cleanup even if a child throws.
  useEffect(() => {
    const prev = document.body.style.overflow;
    try {
      document.body.style.overflow = 'hidden';
    } catch (e) {
      console.warn('[OnboardingModal] Could not set overflow:', e);
    }
    return () => {
      try {
        document.body.style.overflow = prev;
      } catch (e) {
        console.warn('[OnboardingModal] Could not restore overflow:', e);
      }
    };
  }, []);
  ...
  const finish = useCallback(() => {
    console.log('[onboarding] completed, market:', market);
    onComplete(market);
  }, [onComplete, market]);
  ...
  // Fallback: if something inside a step throws, skip gracefully
    ...
    <OnboardingErrorBoundary onError={handleError}>
      ...
      <div className="ob-backdrop" onClick={e => e.target === e.currentTarget && skip()}>
        <div className="ob-card">
          ...
          {step === 3 && <ExplainStep onNext={() => setStep(4)} isMobile={isMobile} />}
          {step === 4 && <ReadyStep   market={market} onFinish={finish} isMobile={isMobile} />}
        </div>
      </div>
