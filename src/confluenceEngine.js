 ...
 * @param {Object}  [params.macroSignal]   — { bias: 'USD_STRONG'|'USD_WEAK', confidence: 0-10 }
 * @param {string}  [params.pair]          — 'EUR/USD' etc (for USD base/quote mapping)
 * @param {number}  [params.stanceScore]   — CB stance for the pair's non-USD leg (-5=dovish, +5=hawkish)
...
export function computeConfluenceScore({
  ...
  // USD_STRONG is bullish for USD-base pairs (USD/JPY, USD/CHF) but bearish
  ...
  if (macroSignal?.bias && typeof macroSignal.confidence === 'number') {
    ...
    macroPts = aligned
      ? Math.round(macroConf * 20)
      : Math.round(-macroConf * 10);
    macroPts = Math.max(-10, Math.min(20, macroPts));
  }
  score += macroPts;
  details.macro = macroPts;

  // ── 4. CB policy stance alignment (max 15 pts, min -8) ───────────────────
  // stanceScore > 0 = hawkish central bank = bullish for their currency.
  let policyPts       = 0;
  let policyAlignment = 'neutral';

  if (typeof stanceScore === 'number' && !isNaN(stanceScore) && stanceScore !== 0) {
    const stanceDir   = stanceScore > 0 ? 'bullish' : 'bearish';
    const aligned     = stanceDir === biasDirection;
    policyAlignment   = aligned ? 'aligned' : 'opposed';
    policyPts = aligned
      ? Math.round((Math.abs(stanceScore) / 5) * 15)
      : Math.round(-(Math.abs(stanceScore) / 5) * 8);
