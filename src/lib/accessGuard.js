 ...
 *
 * FIX: isTrialExpired / hasActiveAccess / getAccessStatus ahora leen
 *      profile.access_type ?? profile.plan para soportar el nombre real
 *      de la columna en users_access (access_type, no plan).
 */
...
export function isTrialExpired(profile) {
  ...
  // FIX: DB column is access_type, not plan — support both for compatibility
  const plan = (profile.access_type ?? profile.plan ?? '').toLowerCase();
...
export function hasActiveAccess(profile, subscription) {
  ...
  // FIX: DB column is access_type, not plan — support both for compatibility
  if ((profile.access_type ?? profile.plan ?? '').toLowerCase() === 'trial' && isTrialExpired(profile)) return false;
